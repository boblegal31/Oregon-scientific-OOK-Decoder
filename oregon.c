...
static uint32_t			RfLinkSeqNumber;
...
{
	register uint32_t	captureDate = LPC_TMR32B0->CR0;

  LPC_TMR32B0->IR = 0x10;			/* clear interrupt flag */

  CaptureEvent.pinStatus = LPC_GPIO1->DATA & 0x20;
...
{
    int temp = 0;
    char buffer[OUTPUT_MESSAGE_LENGTH];
    ...
    sum = snprintf (buffer, OUTPUT_MESSAGE_LENGTH, "RFL 20;%X;OregonV1;ID=00%X;TEMP=%04x;BAT=%s;\r\n", RfLinkSeqNumber++, frame[0] & 0xcf, temp, frame[2] & 0x80?"LOW":"OK");
    USB_WriteEP (CDC_DEP_IN, (unsigned char *)buffer, sum);

}

void DecodeV2V3Frame(const unsigned char * frame, unsigned char length)
{
    unsigned int id = (frame[0] << 8) | frame[1];
    char buffer[OUTPUT_MESSAGE_LENGTH];
    int sum;
    unsigned short	temp, baro, rain, rain_total, wind_dir, wind_speed, average_wind_speed;
    unsigned char	hum, comfort, forecast, uv;
	...
	if(id == 0xea4c || id == 0xca48 || id == 0x0a4d) {
        ...
        sum = snprintf (buffer, OUTPUT_MESSAGE_LENGTH, "RFL 20;%X;Oregon Temp;ID=%X%X;TEMP=%04x;BAT=%s;\r\n", RfLinkSeqNumber++, frame[3], frame[2], temp, frame[4] & 0x80?"LOW":"OK");
        USB_WriteEP (CDC_DEP_IN, (unsigned char *)buffer, sum);
    	...
    	if (id == 0xfa28 || id == 0x1a2d || id == 0x1a3d || (id&0xfff)==0xACC || id == 0xca2c || id == 0xfab8) {
            ...
            sum = snprintf (buffer, OUTPUT_MESSAGE_LENGTH, "RFL 20;%X;Oregon TempHygro;ID=%X%X;TEMP=%04x;HUM=%02X;BAT=%s;\r\n", RfLinkSeqNumber++, frame[1], frame[3], temp, hum, frame[4] & 0x80?"LOW":"OK");
            USB_WriteEP (CDC_DEP_IN, (unsigned char *)buffer, sum);
   	} else
		// ==================================================================================
		...
		if (id == 0x5a6d || id == 0x5a5d || id == 0x5d60) {
            ...
            sum = snprintf (buffer, OUTPUT_MESSAGE_LENGTH, "RFL 20;%X;Oregon BTHR;ID=%X%X;TEMP=%04x;HUM=%02X;HSTATUS=%d;BARO=%04x;BFORECAST=%d;\r\n", RfLinkSeqNumber++, frame[0] & 0xcf, frame[2], temp, hum, comfort, baro, forecast);
            USB_WriteEP (CDC_DEP_IN, (unsigned char *)buffer, sum);
